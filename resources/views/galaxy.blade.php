<x-guest-layout>
    ...
    <div class="cursor-none">
        ...
        <div class="">
            <div class="grid grid-cols-4 mt-32 container mx-auto place-items- gap-10">
                @foreach ($genres as $genre)
                    <div class="w-52 relative hover:scale-105 transition duration-500"
                        style="top:{{ $genre->position_y ?? 0 }}px; left:{{ $genre->position_x ?? 0 }}px">
                        @if ($genre->icon)
                            <img class="planet drop-shadow-2xl" src="{{ $genre->icon }}" />
                        @else
                            <div
                                class="planet shadow-2xl m-auto rounded-full size-40 bg-gradient-to-br from-primary to-secondary">
                            </div>
                        @endif
                    </div>
                @endforeach
