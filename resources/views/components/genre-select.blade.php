    ...
    <div class="relative" x-on:click.outside="open = false">
        <div class="select-none cursor-pointer p-1" x-on:click="open = true">
            <div x-show="genre==null">
                {{ __('Genre') }} @if ($attributes->has('required'))
                    <span class="text-red-500">*</span>
                @endif
            </div>
            <div x-show="genre!=null && icon">
                <img class="size-10" x-bind:src="icon">
            </div>
            <div x-show="genre!=null && !icon" class="font-display text-xs" x-text="label">
            </div>
        </div>
        <div class="absolute bg-secondary-500 py-4 px-6 -right-36 shadow-lg z-50 mt-1 w-72 md:w-96 gap-1  grid grid-cols-2 md:grid-cols-3 rounded-md"
            x-show="open" x-cloak>
